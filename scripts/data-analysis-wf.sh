...
if [  -f $INPUT_DIR/table-dada2.qza ] || [ -f $INPUT_DIR/rooted-tree.qza ]
then
    echo "Alpha rarefaction input files found"
    if [ ! -f $OUTPUT_DIR/alpha-rarefaction.qzv ]
    then
        ...
        echo "Skipping alpha rarefaction"
    fi
fi

if [ -f $INPUT_DIR/core-metrics-results/faith_pd_vector.qza ]
then
    if [ ! -f $OUTPUT_DIR/core-metrics-results/faith-pd-group-significance.qzv ]
    then
        ...
        echo "Skipping alpha group significance - output file exists"
    fi
else
    echo "Skipping alpha group significance - faith pd vector not found"
fi


if [ -f $INPUT_DIR/core-metrics-results/evenness_vector.qza ]
then
    if [ ! -f $OUTPUT_DIR/core-metrics-results/evenness-group-significance.qzv ]
    then
        echo "Running alpha group significance"
        qiime diversity alpha-group-significance \
    ...
    echo "Skipping alpha group significance - evenness vector not found"
fi


if [ -f $INPUT_DIR/core-metrics-results/shannon_vector.qza ]
then
    if [ ! -f $OUTPUT_DIR/core-metrics-results/shannon-group-significance.qzv ]
    then
        echo "Running alpha group significance"
        qiime diversity alpha-group-significance \
        --i-alpha-diversity $INPUT_DIR/core-metrics-results/shannon_vector.qza \
        --m-metadata-file $BARCODES_FILE \
    ...
    echo "Skipping alpha group significance - shannon vector not found"
fi
